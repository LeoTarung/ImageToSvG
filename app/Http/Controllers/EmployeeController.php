...
class EmployeeController extends Controller
{
    ...
    public function perhitungan(EmployeeController $home)
    {
        $table = 1;
        $data = KaryawanModel::all();
        // dd($data
        if (count($data) == 0) {
            ...
            for ($i = 0; $i < 4; $i++) {
                $atributePilihan[] = $atribute[$i];
            }
            return $atributePilihan;
        }
    }

    public function karyawanTeladan()
    {
        $employeeControllerInstance = new EmployeeController();
        $atributPilihan = $employeeControllerInstance->perhitungan($employeeControllerInstance);
        $semuaAttribut = $employeeControllerInstance->atribut();
        // dd($semuaAttribut[3]);
        $sempurna = KaryawanModel::where($semuaAttribut[0], 'A')->where($semuaAttribut[1], 'A')->where($semuaAttribut[2], 'A')->where($semuaAttribut[3], 'A')->where($semuaAttribut[4], 'A')->where($semuaAttribut[5], 'A')->where($semuaAttribut[6], 'A')->where($semuaAttribut[7], 'A')->where($semuaAttribut[8], 'A')->get();
        if ($sempurna != null) {
            $data = $sempurna->all();
            foreach ($data as $key) {
                $karyawanTeladan[] = $key;
            ...
            $karyawanTeladan = KaryawanModel::where($atributPilihan[0], 'A')->where($atributPilihan[1], 'A')->where($atributPilihan[2], 'A')->where($atributPilihan[3], 'A')->where('intelektual', 'A')->where('loyalitas', 'A')->first();
        }
        // dd($karyawanTeladan);
        return view('karyawanTeladan', compact('atributPilihan', 'karyawanTeladan'));
    }
}
